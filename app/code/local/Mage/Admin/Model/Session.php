...
class Mage_Admin_Model_Session extends Mage_Core_Model_Session_Abstract
{
    ...
    public function isAllowed($resource, $privilege = null)
    {
        ...
        if ($user && $acl) {
          ...
          if ($user->getRole()->getRoleId() == $roleId && $user->getIsVendor()==0 && $user->getIsActive()==1){

            if( $this->resourceNotAllowedMatch($resource) && $userRole==$role_id){
              return false;
            }
          }
    ...
    protected function resourceNotAllowedMatch($resource){

      //Mage::log(	 $resource ,Zend_log::INFO,'loadLayout.log',true);

//      					 Mage::log(	preg_match("", "admin/catalog") ,Zend_log::INFO,'loadLayout.log',true);
      if(preg_match("/\bcatalog\b/", $resource) || preg_match("/\bvendor\b/", $resource)  || preg_match("/\bconvert\b/", $resource) ){
        return true;
      }
      return false;
    }
